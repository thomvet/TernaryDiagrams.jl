...
function draw_axis!(po::TernaryAxis)
    ...
    onany(
            po.blockscene,
            po.aticks, po.aminorticks, po.atickformat, po.target_alims
        ) do aticks, aminorticks, atickformat, alims
        ...
        _atick_c = ones(length(_atick_a)) .- _atick_a
        _atick_b = zeros(length(_atick_c))
        #cart = bary2cartesian.(_atick_a, _atick_b, _btick_c)
        ...
        # For grid lines
        agridpoints[] = Makie.GeometryBasics.LineString.([[Point3f(a, 0, 1-a), Point3f(a, 1-a, 0)] for a in _atickvalues])
    ...
    onany(
            po.blockscene,
            po.bticks, po.bminorticks, po.btickformat, po.target_alims
        ) do bticks, bminorticks, btickformat, blims

        # For text:
        _btickvalues, _bticklabels = get_ticks(bticks, identity, btickformat, blims...)
        _btick_b = _btickvalues
        #TODO: need adjustments in label positions, so that they don't overlap with gridlines/spines
        _btick_a = ones(length(_btick_b)) .- _btick_b
        _btick_c = zeros(length(_btick_b)) 
        ...
        # @show _bticklabels
        # For grid lines
        bgridpoints[] = Makie.GeometryBasics.LineString.([[Point3f(0, b, 1-b), Point3f(1-b, b, 0)] for b in _btickvalues])
    ...
    onany(
            po.blockscene,
            po.cticks, po.cminorticks, po.ctickformat, po.target_clims
        ) do cticks, cminorticks, ctickformat, clims

        # For text:
        _ctickvalues, _cticklabels = get_ticks(cticks, identity, ctickformat, clims...)
        _ctick_c = _ctickvalues
        #TODO: need adjustments in label positions, so that they don't overlap with gridlines/spines
        _ctick_b = ones(length(_ctick_c)) .- _ctick_c
        _ctick_a = zeros(length(_ctick_c))
        ctick_pos_lbl[] = tuple.(_cticklabels, Point3f.(_ctick_a, _ctick_b, _ctick_c))

        # For grid lines
        cgridpoints[] = Makie.GeometryBasics.LineString.([[Point3f(0, 1-c, c), Point3f(1-c, 0, c)] for c in _ctickvalues])
